...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def get_multi(
        self, db: Session, *, skip: int = 0, limit: int = -1
    ) -> List[ModelType]:
    ...
    def remove(self, db: Session, *, id: int) -> ModelType:
        ...
        # Instance '<Task at 0x1d0fe10c448>' has been deleted, or its row is otherwise not present
    ...
    def update_list(
        ...
        *,
        ...
        for obj_in in objs_in:
            if isinstance(obj_in, dict):
                update_data = obj_in
                db_obj = self.get(db, obj_in.get('id'))
            else:
                update_data = obj_in.dict(exclude_unset=True)
